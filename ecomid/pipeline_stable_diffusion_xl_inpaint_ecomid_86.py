...
class StableDiffusionXLInpaintPulidPipeline(StableDiffusionXLControlNetPipeline):

    def load_sliders(self, lora_weight=None, device=None):
        ...
        self.network.load_state_dict(torch.load(lora_weight))
    ...
    # def _encode_prompt_image_emb(self, prompt_image_emb, device, batch_size, num_images_per_prompt, dtype,
    ...
    #     prompt_image_emb = prompt_image_emb.reshape([prompt_image_emb.shape[0], -1, self.image_proj_model_in_features])
    #     # prompt_image_emb = prompt_image_emb.repeat(batch_size // prompt_image_emb.shape[0], 1, 1)
    ...
    def prepare_latents(
        ...
        if return_image_latents or (latents is None and not is_strength_max):#true
        ...
        if latents is None and add_noise:
            ...
            # latents = noise if is_strength_max else self.scheduler.add_noise(image_latents, noise, timestep)#is_strength_max 为false。self.scheduler.add_noise(image_latents, noise, timestep)
            # # if pure noise then scale the initial latents by the  Scheduler's init sigma
            # latents = latents * self.scheduler.init_noise_sigma if is_strength_max else latents
            # print("=============================self.scheduler.add_noise===================")
            # print(self.scheduler.add_noise)
            # print("=============================self.scheduler.add_noise()===================")
            # print(inspect.getfile(self.scheduler.add_noise))#/home/galasports/anaconda3/envs/txt2img_86/lib/python3.10/site-packages/diffusers/schedulers/scheduling_dpmsolver_multistep.py
            # time.sleep(100)
            ...
            mask = torch.rand(batch_size, channels, height, width, device=image_latents.device)# 生成在 [0, 1) 上均匀分布的随机数，为每个像素创建采样概率。
            mask = (mask < 0.2).float().half()
             # 将保留的部分替换为image_latents，其他部分用噪声替代
            latents_with_noise = mask * image_latents + (1 - mask) * noise
            latents = latents_with_noise
            ...
            noise = randn_tensor(shape, generator=generator, device=device, dtype=dtype)
            latents = image_latents.to(device) #----wqq-------2024/12/6
            print(self.scheduler.init_noise_sigma)#1.0
            # latents = noise * self.scheduler.init_noise_sigma#-------wqq-------2024/12/6
    ...
    def get_timesteps(self, num_inference_steps, strength, device, denoising_start=None):
        ...
        if denoising_start is None:   
            ...
            t_start = max(num_inference_steps - init_timestep, 0)#30
            print("========================self.scheduler.timesteps=========================")
            print(self.scheduler.timesteps)
            print("========================self.scheduler=========================")
            print(self.scheduler)
            print("========================self.scheduler.order=========================")
            print(self.scheduler.order)#1
            ...
            return timesteps, num_inference_steps - t_start#[30*]
    ...
    def __call__(
            ...
            image_init: PipelineImageInput = None,
        ...
        elif not isinstance(control_guidance_start, list) and not isinstance(control_guidance_end, list):#果 control_guidance_start 和 control_guidance_end 都不是列表类型，则进入该条件。
        ...
        init_image = self.image_processor.preprocess(
            image_init, height=height, width=width, crops_coords=crops_coords, resize_mode=resize_mode
        )
        ...
        masked_image = init_image * (mask < 0.5)#这里的 mask 是一个包含了 [0, 1] 值的张量，通过 mask < 0.5 操作将其转换为布尔值（True 或 False），值小于 0.5 的位置会被转为 True，其余为 False。将 mask 应用到输入图像 init_image 上，得到 masked_image。masked_image 是经过遮罩处理后的图像，只有 mask 中小于 0.5 的区域会被保留，其他区域会被置零。
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = torch.cat([latents] * 2) if self.do_classifier_free_guidance else latents#分一部分对应 无条件输入（Unconditional Input）（负引导），一部分对应 条件输入（Conditional Input）。这样做的目的是让 U-Net 同时预测 无条件噪声 和 条件噪声，方便后续通过公式调整生成结果。
                latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)#调用调度器的方法，将潜在变量标准化到模型的训练分布范围。例如，将潜在变量根据 t 的噪声强度（sigma 值）进行缩放。
                ...
                if guess_mode and self.do_classifier_free_guidance:
                    ...
                    controlnet_prompt_embeds = prompt_embeds.chunk(2)[1]#prompt_embeds 包含无条件和条件两个部分，通过 .chunk(2) 分割后提取条件部分。
                    controlnet_added_cond_kwargs = {
                        "text_embeds": add_text_embeds.chunk(2)[1],#prompt_embeds 包含无条件和条件两个部分，通过 .chunk(2) 分割后提取条件部分。
                ...
                if isinstance(controlnet_keep[i], list):#controlnet_keep，掩码或权重，具体表示在给定时间步中应保留多少条件的影响
                    cond_scale = [c * s for c, s in zip(controlnet_conditioning_scale, controlnet_keep[i])]#cond_scale ：最终计算出来的条件缩放比例。
                else:
                ...
                if isinstance(self.controlnet, MultiControlNetModel):#当存在多个 ControlNet 模型时（例如 MultiControlNetModel），对所有模型的结果进行依次推断并整合：
                ...
                #down_block_additional_residuals 和 mid_block_additional_residual：分别是 ControlNet 的输出，作用于不同的 U-Net 残差块。
                ...
                # perform guidance#将 U-Net 输出的 noise_pred 分为两部分：无条件预测（noise_pred_uncond） ，条件预测（noise_pred_text） 。
                if self.do_classifier_free_guidance:
                ...
                init_latents_proper = image_latents#固定不变，通常是条件生成过程中强引导生成结果的参考潜在变量。
                if self.do_classifier_free_guidance:
                ...
                if i < len(timesteps) - 1:
                    noise_timestep = timesteps[i + 1]#取下一时间步对应的噪声强度。
                    init_latents_proper = self.scheduler.add_noise(
                        init_latents_proper, noise, torch.tensor([noise_timestep])#生成加入下一时间步噪声后的潜在变量；add_noise 方法将当前时间步的噪声注入到 init_latents_proper 中，以确保 init_latents_proper 保持与扩散模型时间步对应的噪声相容性。
                    )
                
                latents = (1 - init_mask) * init_latents_proper + init_mask * latents#####----wqq-gai-12-10
                
                #                 weight 表示扩散过程不同步的动态加权因子：
                # 初始时间步时，weight 较小，此时更多依赖 init_latents_proper。
                # 随时间步增加，weight 趋于较大，此时的 latents 权重更高，即生成潜在状态会逐渐摆脱对输入的完全依赖。
                #----------------------wqq-----------liao-------------------gai--------------------
                # weight = i/len(timesteps)*1.3
                # if i > num_warmup_steps*0.8 and i < len(timesteps) - 4:
                #     latents = (1 - init_mask) * (init_latents_proper * (1-weight) + latents * weight) + init_mask * latents#1 - init_mask 部分：在未被掩码覆盖的区域，latents 和 init_latents_proper 通过权重线性加权融合。init_mask 部分：掩码覆盖的区域直接保留当前的 latents。
                # else:
                #     latents = (1 - init_mask) * init_latents_proper + init_mask * latents
                #-------------------------------------------------------------------------end-----------------
                # ratio = 1 * (i/(len(timesteps)+2 ))
                # latents = (1 - init_mask) * init_latents_proper * ratio + (
                #             1 - init_mask) * latents * (1- ratio) + init_mask * latents
                # latents = (1 - init_mask) * init_latents_proper * 0.9 + (1 - init_mask) * latents * 0.1 + init_mask * latents
                if callback_on_step_end is not None:
                    callback_kwargs = {}#如果存在回调函数，则在每个时间步结束时调用。例如，用户可以自定义回调函数用于保存中间潜在状态、检视生成过程、调试模型、或应用特定操作。
                    for k in callback_on_step_end_tensor_inputs:
                        callback_kwargs[k] = locals()[k]#使用 locals() 将当前函数内的局部变量（如 latents, prompt_embeds）动态封装为字典，并传递给回调函数。
